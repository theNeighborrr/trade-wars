...
export function offerNewRun(){openCompanySetup();}
function clearRunUI(){deskState.quantities={};lastReceipt=null;saveUI();}
function replayRun(){
  confirmAction('Replay this scenario?',`Restart seed ${s().seed} from Day 1 with ${money(s().startingCash)} and the ${house().name} specialty. The same world prices occur on the same days. Your old run is backed up; a daily replay increments this device's attempt count.`,()=>{const result=resetState({replay:true});if(result.ok)clearRunUI();actions(result);callbacks.setView('markets');},'Replay');
}
...
function renderRoutes(){
  ...
  byId('routePlanner').innerHTML=`<summary><span>Route planner</span><small>Compare hubs before traveling</small></summary><div class="route-body">${mapSVG()}<p class="desk-note">Schematic geography · current quotes, not arrival-price forecasts.</p><div class="map-hub-buttons">${hubs.map(h=>`<button class="desk-button ${selectedHub===h.id?'selected':''}" type="button" data-map-hub="${h.id}">${h.code}<small>${esc(h.name)}</small></button>`).join('')}</div><div class="route-quote"><h3>${esc(currentHub().code)} → ${esc(target.code)} · ${esc(target.name)}</h3><div class="desk-kpis"><div><span>Travel cost</span><strong>${here?'—':money(travelCost(target.id))}</strong></div><div><span>Arrival</span><strong>${here?'Current hub':'Day '+Math.min(30,s().day+1)}</strong></div><div><span>Inventory quote difference</span><strong class="${tone(delta)}">${signedMoney(delta)}</strong></div></div><p class="desk-note">${here?'You are here.':`Difference after travel cost at today's quotes: ${signedMoney(delta-travelCost(target.id))}. Prices reprice during travel; this is NOT a guaranteed profit.`}</p><div class="route-buying-quotes">${assets.filter(a=>a.group==='solar').map(a=>`<span>${esc(a.name)} <strong>${exactMoney(localPrice(a.id,target.id))}</strong></span>`).join('')}</div><button class="desk-button accent" id="mapTravel" type="button" ${here||s().day>=30||s().finished||s().cash<travelCost(target.id)?'disabled':''}>Travel · ${money(travelCost(target.id))} · 1 day</button></div></div>`;
...
export function openResults(){
  ...
  const body=openDialog(s().finished?'Campaign complete':'Campaign checkpoint',`<div class="result-hero"><span class="eyebrow">${esc(r.title)}</span><h2>${money(r.worth)}</h2><strong class="${tone(r.returnPct)}">${pct(r.returnPct)} versus ${money(r.startingCash)} starting cash</strong><p>Seed ${esc(r.seed)} · rules ${s().worldRules}</p></div><div class="desk-kpis result-kpis">${[['Cash',money(r.cash)],['Inventory marked at current hub',money(r.inventory)],['Trading profit · realized',signedMoney(r.realized)],['Delivery profit · realized',signedMoney(r.contractProfit)],['Unrealized inventory P/L',signedMoney(r.unrealized)],['Travel expenses',money(r.travelCosts)],['Upgrade expenses',money(r.upgradeCosts||0)],['Forfeited bonds',money(r.forfeits)],['Bonds still held',money(r.escrow)]].map(([name,value])=>`<div><span>${name}</span><strong>${value}</strong></div>`).join('')}</div><p>${r.deliveries} deliveries · ${r.trades} trades · ${r.cities} hubs visited · ${(r.maxDrawdown*100).toFixed(1)}% maximum recorded drawdown.</p><p class="desk-note">${r.migrated?`This is a continued v0.4 campaign. Journal statistics and drawdown cover Day ${r.coverageStart} onward, not earlier activity. It is excluded from v0.5 personal-best comparisons. Replay cannot reconstruct the older random world.`:'Cash + current inventory value + refundable bonds = net worth. Open inventory is not recorded as a sale.'}</p><dl class="driver-list"><div><dt>Best recorded sale</dt><dd>${r.bestSale?esc(getAsset(r.bestSale.assetId).name)+' · '+signedMoney(r.bestSale.realized):'No sales yet'}</dd></div><div><dt>Worst recorded sale</dt><dd>${r.worstSale?esc(getAsset(r.worstSale.assetId).name)+' · '+signedMoney(r.worstSale.realized):'No sales yet'}</dd></div><div><dt>Most profitable sale market</dt><dd>${esc(r.bestAsset||'No profitable sale market yet')}</dd></div><div><dt>Personal best · matching specialty / mode</dt><dd>${best?money(best.worth):'Finish a new v0.6 run to set a record'}</dd></div></dl><div class="desk-dialog-actions"></div>`);
  const a=body.lastElementChild;a.append(button('New scenario',()=>{closeDialog();offerNewRun();},'desk-button accent'));
  if(!s().migrated&&!s().legacyRules)a.append(button('Replay this seed',()=>{closeDialog();replayRun();}));
  a.append(button('Export campaign',()=>download(`trade-wars-${s().seed}.json`,exportCampaign())));
  appendHouseResults(a,r);
}
function help(){
  openDialog('Your trading desk',`<h3>Choose → trade → advance → adapt</h3><p>Tap an asset name or <strong>History / Why?</strong> for price history and drivers. A <strong>local move</strong> compares yesterday and today in the same hub. Global moves exclude hub premiums.</p><p>Drag the slider for quick sizing, or type any exact whole-lot quantity. <strong>Buy Max</strong> and <strong>Sell All</strong> select a quantity only; the stacked BUY / SELL buttons execute it. Both previews show the consequence before you trade.</p><p>Solar / Grid expands into four individually tradable equipment markets. Its parent is an equal-weight, base-100 reference index—not another tradable asset.</p><p>Contract bonds remain part of net worth until refunded or forfeited. Inventory is consumed only on delivery. Deadlines include the final trading session of that day.</p><p>Day 30 stays open for your final trades and deliveries. <strong>Close campaign</strong> ends the run and values unsold goods at that hub's quotes. Unfinished contracts lose their bonds.</p><h3>Trading Houses</h3><p>Manage your company for run-only upgrades and customer trust. Specialties have different bankrolls and starting hubs. Daily challenges lock the Independent setup; first attempts and replays are device-local, self-reported labels. A shared invitation always asks before replacing a run.</p><h3>Terminal keyboard</h3><p><kbd>C</kbd> Command · <kbd>M</kbd> Markets · <kbd>I</kbd> Intel · <kbd>P</kbd> Portfolio · <kbd>J</kbd> Journal · <kbd>N</kbd> next day · <kbd>?</kbd> help. Keys are ignored while typing or a dialog is open. No shortcut executes a trade.</p><p class="desk-note">All themes use the same economy and information. Same seed and rules version = the same world on the same day; outcomes are not guaranteed profits.</p>`);
}
export function initDesk(cb){
  ...
  initHouses({openDialog,closeDialog,confirmAction,button,actions,setView:cb.setView,clearRunUI,openResults,closeCampaign:offerCloseCampaign});
}
