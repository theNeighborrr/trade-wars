import {initUI} from './v05-ui.js?cache=15';
import {RELEASE_LABEL,RELEASE_VERSION,RELEASE_BUILD} from './release.js?cache=15';

function addStyles(){
  for(const href of ['v02.css','trade-controls.css?v=1','mobile-ux.css?cache=14','release.css?cache=11','account-context.css?cache=12','market-layout.css?cache=14','v05-desk.css?cache=15']){
...
function upgradeDOM(){
  ...
  if(topbar&&!document.getElementById('mobileHud')){
    ...
    hud.innerHTML='<div><span>Day</span><strong id="hudDay">1 / 30</strong></div><div><span>Hub</span><strong id="hudHub">NYC</strong></div><div><span>Cash</span><strong id="hudCash">$50,000</strong></div><div><span>Net worth</span><strong id="hudWorth">$50,000</strong></div>';
    topbar.after(hud);
  }

  const account=document.querySelector('.sidebar-account-grid');
  if(account&&!document.getElementById('sideBonds')){const bonds=document.createElement('div');bonds.innerHTML='<span>Contract bonds</span><strong id="sideBonds">$0</strong>';account.append(bonds);}
  ...
  if(markets&&!document.getElementById('hubGrid')){
    ...
    if(p)p.textContent='Buy in one hub, travel, and sell into another market. Travel uses one day; prices and ongoing conditions evolve en route.';
  ...
  if(!document.getElementById('dayTransition')){
    ...
    transition.innerHTML='<div class="day-transition-card"><div class="day-transition-day" id="dayTransitionDay">DAY 2</div><h2>Markets repriced</h2><button class="desk-button" type="button" id="dismissTurn">Continue</button><p id="dayTransitionTitle">New world event</p><div class="day-transition-movers" id="dayTransitionMovers"></div></div>';
