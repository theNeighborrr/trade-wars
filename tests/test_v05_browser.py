...
parser=argparse.ArgumentParser();parser.add_argument('--quick',action='store_true');parser.add_argument('--report',default='v05-browser-report.json');args=parser.parse_args()
...
def engine(page,code):return page.evaluate("async()=>{const g=await import(window.__twModules['v05-engine.js']);"+code+"}")
def dismiss(page):
...
with sync_playwright() as pw:
 ...
 for width in [390,700,1024,1640]:
  ...
  saved=p.evaluate("()=>localStorage.getItem('trade-wars-campaign-v5')");q=b.new_page(viewport={'width':width,'height':950});mount(q,ROOT,'terminal',{'trade-wars-campaign-v5':saved});assert engine(q,'return g.getState();')==json.loads(saved);q.close()
