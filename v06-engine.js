import {STARTING_CASH,MAX_DAYS,RULES,STORAGE_KEY,LEGACY_KEY,V5_KEY,RECORDS_KEY,ATTEMPTS_KEY,assets,hubs,conditions,contractTemplates,specialties,customers,upgrades,getSpecialty,getCustomer,customerTier,utcDay,validChallengeDate,dailySeed,shareURL} from './v06-data.js?cache=16';
import {contractTemplates as oldTemplates} from './v05-data.js?cache=15';
...
export function randomAt(seed,key,rules=RULES){
...
export function createGame({storage=browserStorage(),seed,now=()=>new Date()}={}){
  ...
  function fresh(seedValue,options={}){
    const specialty=options.mode==='daily'?specialties[0]:(getSpecialty(options.specialty)||specialties[0]);
    const companyName=String(options.companyName||'Independent Trading Co.').normalize('NFKC').replace(/[\u0000-\u001f\u007f]/g,'').trim().slice(0,40)||'Independent Trading Co.';
    return {version:6,rules:RULES,worldRules:RULES,legacyRules:false,startingCash:specialty.cash,
      mode:options.mode==='daily'?'daily':'free',challengeDate:options.challengeDate||null,attempt:options.attempt??null,
      company:{name:companyName,specialty:specialty.id,home:specialty.home,upgrades:[],customers:Object.fromEntries(customers.map(c=>[c.id,{trust:0,delivered:0,failed:0}]))},seed:seedText(seedValue),runId:`run-${Date.now()}-${Math.random().toString(36).slice(2,9)}`,day:1,worldStartDay:1,coverageStart:1,migrated:false,cash:specialty.cash,
      ...
      currentHub:specialty.home,moveReasons:{},wealthHistory:[specialty.cash],priceHistory:[],activeConditions:[],eventHistory:[],journal:[],travelHistory:[],offers:[],contracts:[],escrow:0,finished:false,result:null,
      lastTurn:null,openingEquity:specialty.cash,openingUnrealized:0,sequence:0};
  }
  function basicValid(s){
    if(!s||![2,5,6].includes(s.version)||!Number.isInteger(s.day)||s.day<1||s.day>MAX_DAYS||!Number.isFinite(s.cash)||s.cash<-.00001||!s.prices||!s.holdings||!s.avgCosts)return false;
  ...
  function validCampaign(s){
    return basicValid(s)&&[5,6].includes(s.version)&&s.rules===(s.version===5?'0.5.0':RULES)&&typeof s.seed==='string'&&hubById(s.currentHub)
  ...
  function validCompany(s){
    return s.version===6&&validCampaign(s)&&getSpecialty(s.company?.specialty)&&typeof s.company.name==='string'&&s.company.name.length<=40
      &&Array.isArray(s.company.upgrades)&&new Set(s.company.upgrades).size===s.company.upgrades.length&&s.company.upgrades.every(id=>upgrades.some(u=>u.id===id))
      &&customers.every(c=>{const r=s.company.customers?.[c.id];return r&&Number.isInteger(r.trust)&&r.trust>=0&&r.trust<=6&&Number.isInteger(r.delivered)&&r.delivered>=0&&Number.isInteger(r.failed)&&r.failed>=0;})
      &&['free','daily'].includes(s.mode)&&['0.5.0',RULES].includes(s.worldRules)&&Number.isFinite(s.startingCash)&&s.startingCash>0
      &&(s.mode!=='daily'||(validChallengeDate(s.challengeDate,utcDay(now()))&&s.company.specialty==='independent'&&s.seed===dailySeed(s.challengeDate)&&(s.attempt===null||Number.isSafeInteger(s.attempt)&&s.attempt>0)));
  }
  let state;
  const rng=(key)=>randomAt(state.seed,key,state.worldRules||RULES);
  const house=()=>getSpecialty(state.company.specialty)||specialties[0];
  const maxContracts=()=>state.company.upgrades.includes('operations')?4:3;
  function travelCost(id){const h=hubById(id);if(!h)return NaN;return state.legacyRules?h.travelCost:Math.round(h.travelCost*house().travelFactor*(state.company.upgrades.includes('carrier')?.8:1));}
  function nextTenderDay(){const step=5-((state.day-state.worldStartDay)%5);const next=state.day+step;return next<=24?next:null;}
  function relationship(id){return state.company.customers[id]||{trust:0,delivered:0,failed:0};}
  function noteCustomer(id,success){
    if(state.legacyRules||!getCustomer(id))return;
    const r=state.company.customers[id];r.trust=clamp(r.trust+(success?1:-1),0,6);r[success?'delivered':'failed']++;
  }
  function quoteUpgrade(id){
    const u=upgrades.find(x=>x.id===id);if(!u)return {ok:false,message:'Unknown upgrade.'};
    if(state.legacyRules)return {ok:false,message:'This preserved campaign keeps its original rules. Start a v0.6 company to buy upgrades.'};
    if(state.finished||state.day>=30)return {ok:false,message:'Upgrades are closed in the final session.'};
    if(state.company.upgrades.includes(id))return {ok:false,message:'Already purchased for this run.'};
    if(id==='procurement'&&!nextTenderDay())return {ok:false,message:'No future tender rounds remain in this campaign.'};
    if(state.cash<u.cost)return {ok:false,message:`Requires ${money(u.cost)} in available cash.`};
    return {ok:true,...u,cashAfter:state.cash-u.cost,nextTender:nextTenderDay()};
  }
  function purchaseUpgrade(id){
    const p=quoteUpgrade(id);if(!p.ok)return p;
    state.cash=p.cashAfter;state.company.upgrades.push(id);
    const receipt=record('upgrade',{upgradeId:id,total:p.cost,cashAfter:state.cash,message:p.name});mark();save();
    return {ok:true,receipt,message:`${p.name} purchased for ${money(p.cost)}. Benefits apply to future operations.`};
  }
  ...
  function worldConditions(day){
    const order=[...conditions].sort((a,b)=>rng(`deck/${a.id}`)-rng(`deck/${b.id}`));
    const out=[];
    for(let start=state.worldStartDay,i=0;start<=day;start+=3,i++){
      ...
      else if(rng(`outcome/${start}/${def.id}`)>=def.chance){
  ...
  function legacyOfferBatch(){
    if(state.day>24||!((state.day-state.worldStartDay)%5===0))return;
    state.offers=[];
    oldTemplates.forEach((template,i)=>{
      const h=hubs[Math.floor(rng(`contract/${state.day}/${i}/hub`)*hubs.length)];
      const scale=1+Math.floor(rng(`contract/${state.day}/${i}/size`)*2);
  ...
  function offerBatch(){
    if(state.legacyRules){legacyOfferBatch();return;}
    if(state.day>24||((state.day-state.worldStartDay)%5!==0))return;
    const round=Math.floor((state.day-state.worldStartDay)/5);
    state.offers=[];
    const candidates=customers.filter(c=>relationship(c.id).delivered>0&&relationship(c.id).trust>0);
    const rush=candidates.length?candidates[Math.floor(rng(`rush/${state.day}`)*candidates.length)]:null;
    for(let i=0;i<3;i++){
      let template=contractTemplates[(round+i)%contractTemplates.length],kind='standard';
      if(i===1&&rush){template=contractTemplates.find(t=>t.customerId===rush.id);kind='rush';}
      if(i===2&&state.company.upgrades.includes('procurement'))kind='licensed';
      const h=hubs[Math.floor(rng(`contract/${state.day}/${i}/hub`)*hubs.length)];
      const scale=kind==='rush'?1:kind==='licensed'?3:1+Math.floor(rng(`contract/${state.day}/${i}/size`)*2);
      const requirements=Object.fromEntries(Object.entries(template.requirements).map(([id,n])=>[id,n*scale]));
      const quote=sum(Object.entries(requirements).map(([id,n])=>n*localPrice(id,h.id)));
      const trust=relationship(template.customerId).trust;
      const bonus=house().clientIds.includes(template.customerId)?house().clientBonus:0;
      const rate=kind==='rush'?1.42:kind==='licensed'?1.24*1.08:1.24;
      const payout=Math.round((quote*rate+h.travelCost*.65)*(1+bonus));
      const bondFactor=house().bondFactor*(trust>=3?.8:trust>=1?.9:1);
      const bond=Math.max(50,Math.round(payout*.05*bondFactor));
      state.offers.push({id:`C-${state.day}-${i+1}`,name:(kind==='rush'?'Rush: ':kind==='licensed'?'Major order: ':'')+template.name,
        customerId:template.customerId,kind,hub:h.id,requirements,payout,bond,created:state.day,expires:Math.min(28,state.day+(kind==='rush'?1:3)),
        deadline:Math.min(MAX_DAYS,state.day+(kind==='rush'?2:7+i)),status:'offered',terms:{specialtyBonus:bonus,bondFactor,trust,baseGoodsQuote:quote}});
    }
  }
  function initialize(){
  ...
  function migrateV5(old){
    const next=copy(old),base=fresh(old.seed);
    next.version=6;next.rules=RULES;next.worldRules='0.5.0';next.legacyRules=true;
    next.startingCash=STARTING_CASH;next.company=base.company;next.mode='free';next.challengeDate=null;next.attempt=null;
    next.company.name='Continued Trading Co.';next.result=null;
    return next;
  }
  const raw=read(STORAGE_KEY),v5=read(V5_KEY),legacy=read(LEGACY_KEY);
  try{
    const parsed=raw?JSON.parse(raw):null;
    if(parsed&&!validCompany(parsed))throw Error('Unrecognized save');
    if(parsed)state=parsed;
    else if(!seed&&v5){
      const old=JSON.parse(v5);if(!validCampaign(old)||old.version!==5)throw Error('Invalid v0.5 save');
      state=migrateV5(old);save();
      loadNotice='Your v0.5 campaign is preserved, including its original world, contracts and history. Company perks and daily challenges apply to new v0.6 runs. The original save is untouched.';
    }else if(!seed&&legacy){
      const old=JSON.parse(legacy);if(!basicValid(old)||old.version!==2)throw Error('Invalid legacy save');
      state=migrate(old);state.worldRules='0.5.0';state.legacyRules=true;state.openingEquity=netWorth();state.openingUnrealized=unrealized();mark();initialize();
      loadNotice=`Campaign preserved from v0.4. Journal and history begin on Day ${state.day}. Company perks apply to new runs; the original save is untouched.`;
  ...
  function reprice(){
    ...
    assets.forEach(a=>{
      const noise=(rng(`noise/${state.day}/${a.id}`)-.5)*2*a.volatility*.65;
    ...
    hubs.forEach(h=>assets.forEach(a=>{
      state.hubDrift[h.id][a.id]=clamp((state.hubDrift[h.id][a.id]||1)*.985+.015+(rng(`hub/${state.day}/${h.id}/${a.id}`)-.5)*.014,.93,1.07);
  ...
  function expireContracts(){
    state.contracts.filter(c=>c.status==='accepted'&&state.day>c.deadline).forEach(c=>{
      c.status='expired';noteCustomer(c.customerId,false);state.escrow=Math.max(0,state.escrow-c.bond);record('forfeit',{contractId:c.id,total:c.bond,message:`Deadline missed: ${c.name}`});
    });
  ...
  function travelTo(hubId){
    ...
    const cost=travelCost(hubId);
    if(state.cash<cost)return {ok:false,message:`Travel requires ${money(cost)} in available cash.`};
    const from=state.currentHub;state.cash-=cost;state.currentHub=hubId;
    const entry={day:state.day+1,from:hubById(from).name,to:h.name,cost};state.travelHistory.push(entry);
    record('travel',{hub:from,fromHub:from,toHub:hubId,total:cost,baseCost:h.travelCost,arrivalDay:state.day+1});
    return advanceDay({fromHub:from,travelTo:hubId,travelCost:cost});
  }
  function acceptContract(id){
    ...
    if(state.contracts.filter(c=>c.status==='accepted').length>=maxContracts())return {ok:false,message:`Complete or cancel a contract first (${maxContracts()} active maximum).`};
  ...
  function fulfillContract(id){
    ...
    state.cash+=c.payout+c.bond;state.escrow=Math.max(0,state.escrow-c.bond);c.status='fulfilled';c.completed=state.day;noteCustomer(c.customerId,true);
  ...
  function cancelContract(id){
    ...
    c.status='cancelled';noteCustomer(c.customerId,false);state.escrow=Math.max(0,state.escrow-c.bond);record('forfeit',{contractId:id,total:c.bond,message:`Cancelled: ${c.name}`});mark();save();
  ...
  function campaignReport(){
    ...
    const upgradeCosts=sum(state.journal.filter(x=>x.type==='upgrade').map(x=>x.total));
    const worth=netWorth(),returnPct=worth/state.startingCash-1;
    ...
    return {runId:state.runId,seed:state.seed,rules:state.worldRules,companyName:state.company.name,specialty:state.company.specialty,mode:state.mode,challengeDate:state.challengeDate,attempt:state.attempt,startingCash:state.startingCash,legacyRules:state.legacyRules,upgradeCosts,worth,returnPct,realized,contractProfit,travelCosts,forfeits,inventory:holdingsValue(),escrow:state.escrow,unrealized:unrealized(),cash:state.cash,bestSale,worstSale,bestAsset:bestAsset?.name||null,byAsset,title,deliveries:deliveries.length,trades:state.journal.filter(x=>['buy','sell'].includes(x.type)).length,cities:new Set([state.currentHub,...state.journal.filter(x=>x.type==='travel').flatMap(x=>[x.fromHub,x.toHub])]).size,maxDrawdown,migrated:state.migrated,coverageStart:state.coverageStart,openingEquity:state.openingEquity,openingUnrealized:state.openingUnrealized};
  }
  function allRecords(){try{const r=JSON.parse(read(RECORDS_KEY)||'[]');return Array.isArray(r)?r.filter(x=>x.rules===RULES&&Number.isFinite(x.worth)):[];}catch{return [];}}
  function getRecords(){return allRecords().filter(r=>r.specialty===state.company.specialty&&r.mode===state.mode&&(state.mode!=='daily'||r.challengeDate===state.challengeDate)).sort((a,b)=>b.worth-a.worth);}
  function finishCampaign(){
    ...
    for(const c of state.contracts.filter(c=>c.status==='accepted')){c.status='expired';noteCustomer(c.customerId,false);state.escrow=Math.max(0,state.escrow-c.bond);record('forfeit',{contractId:c.id,total:c.bond,message:`Campaign closed before delivery: ${c.name}`});}
    mark();state.finished=true;state.result=campaignReport();
    if(!state.migrated&&!state.legacyRules){
      const records=allRecords().filter(r=>r.runId!==state.runId);
      const {runId,seed,rules,worth,returnPct,title,specialty,mode,challengeDate,attempt,companyName}=state.result;
      records.push({runId,seed,rules,worth,returnPct,title,specialty,mode,challengeDate,attempt,companyName});
      write(RECORDS_KEY,JSON.stringify(records.slice(-200)));
    }
    save();return {ok:true,finished:true,message:`Campaign complete · ${money(state.result.worth)} · ${pct(state.result.returnPct)}`};
  }
  function nextDailyAttempt(date){
    let all={};try{const parsed=JSON.parse(read(ATTEMPTS_KEY)||'{}');if(parsed&&typeof parsed==='object'&&!Array.isArray(parsed))all=parsed;}catch{}
    const key=`${RULES}/${date}`,last=Number.isSafeInteger(all[key])&&all[key]>=0?all[key]:0;
    const attempt=last+1;all[key]=attempt;
    return write(ATTEMPTS_KEY,JSON.stringify(all))?attempt:null;
  }
  function resetState({seed:nextSeed,replay=false,specialty,companyName,mode='free',challengeDate}={}){
    if(replay&&(state.migrated||state.legacyRules))return {ok:false,message:'This preserved campaign uses an older world. Start a new v0.6 scenario.'};
    if(replay){nextSeed=state.seed;specialty=state.company.specialty;companyName=state.company.name;mode=state.mode;challengeDate=state.challengeDate;}
    if(!['free','daily'].includes(mode)||specialty&&!getSpecialty(specialty))return {ok:false,message:'Unknown campaign type or specialty.'};
    if(mode==='daily'&&!validChallengeDate(challengeDate,utcDay(now())))return {ok:false,message:'Daily challenge date is invalid or not available yet.'};
    // Protect the only in-memory copy before destructive new/replay operations.
    if(!write(`${STORAGE_KEY}-previous`,JSON.stringify(state)))return {ok:false,message:'Could not back up this campaign. Export it before starting another run.'};
    const options={specialty,companyName,mode,challengeDate:mode==='daily'?challengeDate:null};
    if(mode==='daily'){options.specialty='independent';options.attempt=nextDailyAttempt(challengeDate);nextSeed=dailySeed(challengeDate);}
    state=fresh(nextSeed,options);loadNotice='';initialize();return {ok:true,message:`Fresh ${mode==='daily'?'daily challenge':'company'} · seed ${state.seed}`};
  }
  function nextDecisions(){
    if(state.finished)return [{kind:'results',priority:0,title:'Campaign complete',detail:'Review the final accounts or share your result.',action:'View results'}];
    const list=[];
    for(const c of state.contracts.filter(c=>c.status==='accepted').sort((a,b)=>a.deadline-b.deadline)){
      const left=c.deadline-state.day,missing=Object.entries(c.requirements).map(([id,n])=>({id,qty:Math.max(0,n-state.holdings[id])})).filter(x=>x.qty>0);
      const sourcing=sum(missing.map(x=>x.qty*localPrice(x.id))),fare=c.hub===state.currentHub?0:travelCost(c.hub),gap=Math.max(0,sourcing+fare-state.cash);
      const atHub=c.hub===state.currentHub,ready=atHub&&!missing.length;
      let detail=ready?'All goods are ready here.':missing.length?'Need '+missing.map(x=>`${x.qty} ${assetById(x.id).ticker}`).join(' + ')+'.':'All required goods held.';
      if(!atHub)detail+=` Travel to ${hubById(c.hub).code}: ${money(fare)} + 1 day.`;
      if(left===0&&!atHub)detail+=' Arrival would miss the deadline.';
      else if(gap>.005)detail+=` Cash short ${money(gap)} for missing goods + travel at current quotes.`;
      else if(!ready)detail+=' Cash covers missing goods + travel at current quotes; arrival prices may change.';
      list.push({kind:'contract',id:c.id,priority:ready?0:left<=1?1:3,title:`${ready?'Ready to deliver':left===0?'Due today':'Due in '+left+' days'} · ${c.name}`,detail,action:ready?'Deliver / review':'Review contract'});
    }
    for(const c of state.activeConditions.filter(c=>c.phase==='developing'&&c.remaining<=1))list.push({kind:'intel',priority:2,title:`Decision next session · ${c.name}`,detail:'Developing, not guaranteed. '+Object.keys(c.effects).map(id=>assetById(id)?.ticker||id).join(' · '),action:'Read Intel'});
    if(state.day>=29)list.push({kind:'closing',priority:state.day===30?0:2,title:state.day===30?'Final trading session':'One market session remains',detail:'Finish trades and deliveries before closing. Unfinished contract bonds will be forfeited.',action:state.day===30?'Close campaign':'Review commitments'});
    if(!list.length)list.push({kind:'contract',priority:5,title:'No delivery deadlines yet',detail:'Explore optional customer offers, or keep your cash free for ordinary trading.',action:'Review offers'});
    return list.sort((a,b)=>a.priority-b.priority).slice(0,3);
  }
  function shareResult(){
    const r=state.result||campaignReport();
    if(state.legacyRules)return {ok:false,message:'This preserved ruleset cannot create a matching v0.6 challenge link.'};
    const url=shareURL({challengeDate:state.challengeDate,seed:state.seed,specialty:state.company.specialty,rules:state.worldRules});
    const attempt=state.mode==='daily'?(state.attempt===1?'First attempt on this device':state.attempt?'Replay · attempt '+state.attempt:'Attempt not tracked'):'Free campaign';
    const text=`TRADE WARS · ${state.finished?'FINAL RESULT':'CHECKPOINT · Day '+state.day}\n${state.company.name} · ${house().name}\n${state.mode==='daily'?'Daily challenge '+state.challengeDate+' (UTC)':'Seed '+state.seed}\n${money(r.worth)} net worth · ${pct(r.returnPct)} on ${money(state.startingCash)}\n${r.deliveries} deliveries · ${attempt}\nSelf-reported, device-local score; not server verified.\nPlay the same starting setup:`;
    return {ok:true,title:'Trade Wars · Trading Houses',text,url};
  }
  return {getState:()=>state,house,maxContracts,travelCost,nextTenderDay,relationship,quoteUpgrade,purchaseUpgrade,nextDecisions,shareResult,currentHub,hubFactor,localPrice,localMove,holdingsValue,netWorth,investedCost,unrealized,previewTrade,trade,advanceDay,travelTo,acceptContract,deliveryPreview,fulfillContract,cancelContract,finishCampaign,resetState,campaignReport,getRecords,getStorageWarning:()=>storageWarning,getLoadNotice:()=>loadNotice,exportCampaign:()=>JSON.stringify(state,null,2)};
}
...
export const house=()=>game.house(),maxContracts=()=>game.maxContracts(),travelCost=(...a)=>game.travelCost(...a),nextTenderDay=()=>game.nextTenderDay(),relationship=(...a)=>game.relationship(...a),quoteUpgrade=(...a)=>game.quoteUpgrade(...a),purchaseUpgrade=(...a)=>game.purchaseUpgrade(...a),nextDecisions=()=>game.nextDecisions(),shareResult=()=>game.shareResult();
