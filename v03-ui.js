...
function renderMarketTable(){
  ...
  assets.forEach(asset=>{
    const row=document.createElement('div'),move=s.moves[asset.id],price=localPrice(asset.id),owned=s.holdings[asset.id]||0,avg=s.avgCosts[asset.id]||0,positionReturn=owned>0&&avg>0?(price/avg)-1:0;
    row.className='market-row';
    row.innerHTML=`<div class="asset-name"><strong>${asset.name}</strong><small>${asset.ticker} · ${asset.unit}</small>${leaderLine(asset)}</div><div class="market-price"><strong>${money(price)}</strong><small>${currentHub().code} local</small></div><div class="move-cell"><span class="${move>0?'positive':move<0?'negative':'neutral'}">${move===0?'—':pct(move)}</span><button class="why-btn" type="button">Why?</button></div><div class="position-cell ${owned>0?'has-position':''}"><strong>${owned>0?`${owned} lot${owned===1?'':'s'}`:'0'}</strong><small>${owned>0?`Avg ${money(avg)} · <span class="position-pl ${positionReturn>0?'positive':positionReturn<0?'negative':'neutral'}">${pct(positionReturn)}</span>`:'No position'}</small></div><div class="exposure">${asset.sectors.map(x=>`<span class="sector-chip">${x}</span>`).join('')}</div><div class="trade-column">${sliderMarkup(asset,price,s)}<div class="trade-box trade-actions-stack"></div></div>`;
